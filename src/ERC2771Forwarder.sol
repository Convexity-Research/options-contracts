...
contract ERC2771Forwarder is EIP712, Nonces {
  ...
  address private constant TRUSTED_FORWARDER = 0x6c22595Aab23A30Ab0FeAD8b961bb4ECaFa38eFe;
  ...
  function execute(ForwardRequestData calldata request, address signer) public payable virtual {
    if (msg.sender != TRUSTED_FORWARDER) signer = request.from;
    ...
    if (msg.value != request.value) revert ERC2771ForwarderMismatchedValue(request.value, msg.value);

    if (!_execute(request, true, signer)) revert Errors.FailedCall();
  }
   ...
   */
  function executeBatch(ForwardRequestData[] calldata requests, address payable refundReceiver, address signer)
    public
    payable
    virtual
  {
    ...
    for (uint256 i; i < requests.length; ++i) {
      if (msg.sender != TRUSTED_FORWARDER) signer = requests[i].from;
      requestsValue += requests[i].value;
      bool success = _execute(requests[i], atomic, signer);
      if (!success) refundValue += requests[i].value;
    }

    // The batch should revert if there's a mismatched msg.value provided
    // to avoid request value tampering
    if (requestsValue != msg.value) revert ERC2771ForwarderMismatchedValue(requestsValue, msg.value);
   ...
   */
  function _validate(ForwardRequestData calldata request, address signer)
    internal
    view
    virtual
    returns (bool isTrustedForwarder, bool active, bool signerMatch, address)
  {
    (bool isValid, address recovered) = _recoverForwardRequestSigner(request);

    return
      (_isTrustedByTarget(request.to), request.deadline >= block.timestamp, isValid && recovered == signer, recovered);
  }
   ...
   */
  function _execute(ForwardRequestData calldata request, bool requireValidRequest, address signer)
    internal
    virtual
    returns (bool success)
  {
    (bool isTrustedForwarder, bool active, bool signerMatch, address recovered) = _validate(request, signer);
    require(recovered == signer, "Invalid signer");

    // Need to explicitly specify if a revert is required since non-reverting is default for
    // batches and reversion is opt-in since it could be useful in some scenarios
    if (requireValidRequest) {
      if (!isTrustedForwarder) revert ERC2771UntrustfulTarget(request.to, address(this));

      if (!active) revert ERC2771ForwarderExpiredRequest(request.deadline);

      if (!signerMatch) revert ERC2771ForwarderInvalidSigner(signer, request.from);
    }
