...
contract MarketNew is IMarket, Initializable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  TakerQ[][4] private takerQ; // 4 buckets
  ...
  function withdrawCollateral(uint256 amount, bytes memory signature) external isValidSignature(signature) {
    address trader = _msgSender();
    whitelist[trader] = true;
  ...
  function _placeOrder(
    ...
    if (userAccounts[trader].balance < _requiredMargin(trader, _getOraclePrice(), MM_BPS)) {
  ...
  function _marketOrder(MarketSide side, uint128 want, address taker) private {
    ...
    while (left > 0) {
      ...
      while (left > 0 && nodeId != 0) {
        ...
        uint128 taken = uint128(_settleFill(
          nodeId,
          side,
  ...
  function _requiredMargin(address trader, uint256 price, uint256 marginBps) internal view returns (uint256) {
